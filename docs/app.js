...
const safeValue = (id, val) => { const el = $(id); if (el) el.value = val; };
...
function getRootMeta(){
  return state.runMeta || state.latestMeta || null;
}

function hasSimpleSeasonalSchema(){
  const per = state?.meta?.paths?.per_time || {};
  return !!(per.pcatch && per.phab && per.ops);
}

function setOptionVisibility(selectId, keepValues){
  const sel = $(selectId);
  if(!sel) return;
  const keep = new Set(keepValues || []);
  let firstVisible = null;
  Array.from(sel.options).forEach(opt=>{
    const show = keep.has(opt.value);
    opt.hidden = !show;
    opt.disabled = !show;
    if(show && firstVisible === null) firstVisible = opt.value;
  });
  if(firstVisible !== null && (sel.selectedOptions.length===0 || sel.selectedOptions[0].disabled || sel.selectedOptions[0].hidden)){
    sel.value = firstVisible;
  }
}

function setStepOptions(mode){
  const sel = $("stepSelect");
  if(!sel) return;
  const current = sel.value;
  if(mode === "frames") {
    sel.dataset.mode = "frames";
    sel.innerHTML = `
    ...
    sel.dataset.mode = "hours";
    sel.innerHTML = `
      <option value="6" selected>6h</option>
      <option value="12">12h</option>
      <option value="24">24h</option>`;
    if(["6","12","24"].includes(String(current))) sel.value = String(current);
  }
}

function syncAnnualViewerUi(){
  const rootMeta = getRootMeta();
  const spec = rootMeta?.temporal_spec || null;
  const seasonal = spec?.mode === "seasonal_recurring";
  const box = $("seasonalBox");
  if(box) box.hidden = !seasonal;

  if(seasonal) {
    safeValue("seasonalModeBox", "Seasonal recurring");
    safeValue("seasonalTimezoneBox", spec.timezone_label || spec.timezone || "—");
    safeValue("seasonalYearsBox", `${spec.start_year} → ${spec.end_year}`);
    safeValue("seasonalWindowBox", `${spec.season_start_mmdd} → ${spec.season_end_mmdd}`);
    safeValue("seasonalStepBox", `${spec.step_value} ${spec.step_unit}`);
    safeValue("seasonalSnapshotBox", `Local ${String(spec.snapshot_local_hour).padStart(2,'0')}:00`);
    setStepOptions("frames");
    if($("avgToggle")){ $("avgToggle").checked = false; $("avgToggle").disabled = true; }
    if($("lookbackSelect")) $("lookbackSelect").disabled = true;
  } else {
    if($("avgToggle")) $("avgToggle").disabled = false;
    if($("lookbackSelect")) $("lookbackSelect").disabled = false;
    setStepOptions("hours");
  }

  if(hasSimpleSeasonalSchema()) {
    setOptionVisibility("mapSelect", ["pcatch","phab","pops"]);
    if($("modelSelect")) $("modelSelect").disabled = true;
  } else {
    setOptionVisibility("mapSelect", ["pcatch","phab","pops","agree","spread","conf"]);
...
async function showPointPopup(lat, lon, metaInfo){
  try{
    ...
    if(singleMode && timeId && state?.meta?.paths?.per_time){
      ...
      const wantKeys = hasSimpleSeasonalSchema() ? ["pcatch","phab","ops"] : ["pcatch_ensemble","phab_scoring","pops"];
      const parts = [];
...
function currentPerTimeKey(){
  ...
  const per = state?.meta?.paths?.per_time || {};
  if(hasSimpleSeasonalSchema()) {
    if(mapKey === "pcatch") return "pcatch";
    if(mapKey === "phab") return "phab";
    if(mapKey === "pops") return "ops";
    return "pcatch";
  }
...
function renderProfile(){
  ...
  if(!sp){
    const lbl = state.meta?.label?.en || state.meta?.species || "—";
    const sci = state.meta?.scientific_name || "";
    $("profileBox").innerHTML = `<div><b>${lbl}</b>${sci ? ` • <span class="muted">${sci}</span>` : ""}</div><div class="muted small">Seasonal run metadata is available, but the full explainable species profile was not written in this bundle.</div>`;
    return;
  }
  const p = sp.priors;
...
async function loadCovAtPoints(timeIso, points){
  const per = state?.meta?.paths?.per_time || {};
  if(!(per.sst && per.chl && per.current && per.waves)) return {sst:[], chl:[], current:[], waves:[]};
...
async function getConfAggregated(timeIsos){
  const per = state?.meta?.paths?.per_time || {};
  if(!per.conf){
    const conf = new Float32Array(state.grid.width * state.grid.height);
    for(let i=0;i<conf.length;i++) conf[i] = (!state.analysisMask || state.analysisMask[i]!==0) ? 1 : 0;
    return conf;
  }
  ...
  if(state.qcOn && per.qc_chl){
    ...
    for(let i=0;i<conf.length;i++){
      ...
      let s=0, k=0;
      for(let t=0;t<qcArrs.length;t++){ s += (qcArrs[t][i] > 0) ? 1 : 0; k++; }
      qcMean[i] = (k>0)? (s/k) : 1;
    }
...
async function refreshMeta(){
  state.latestMeta = await fetchJson("latest/meta.json").catch(()=>null);
  let idx = await fetchJson("latest/index.json").catch(()=> fetchJson("latest/meta_index.json").catch(()=>null));
  if(!idx && state.latestMeta){
    idx = {
      version: 1,
      generated_at_utc: state.latestMeta.generated_at_utc,
      latest_run_id: state.latestMeta.run_id,
      runs: [{
        run_id: state.latestMeta.run_id,
        path: state.latestMeta.run_path,
        variants: [state.latestMeta.variant],
        species: state.latestMeta.species || [],
        models: state.latestMeta.models || [],
        time_ids: state.latestMeta.available_time_ids || []
      }]
    };
  }
  if(!idx || !idx.runs || !idx.runs.length) throw new Error("No run index found under latest/");
  ...
  state.runId = state.latestMeta?.run_id || state.index.latest_run_id || state.index.runs[state.index.runs.length-1]?.run_id;
...
async function refreshVariants(){
  ...
  const preferred = $("gapToggle").checked ? "gapfill" : (state.latestMeta?.variant || variants[0]);
  state.variant = variants.includes(preferred) ? preferred : (variants[0] || state.latestMeta?.variant);
...
async function loadSpeciesMetaAndInit(){
  ...
  if(bbox && bbox.length===4){
    state.grid.lon_min = bbox[0];
    state.grid.lat_min = bbox[1];
    state.grid.lon_max = bbox[2];
    state.grid.lat_max = bbox[3];
    state.grid.bbox = bbox.slice();
  }
...
function startPlay(){
  ...
  let busy = false;
  const stepValue = Number($("stepSelect")?.value || 1);
  const stepN = ($("stepSelect")?.dataset.mode === "frames") ? Math.max(1, stepValue) : Math.max(1, Math.round(stepValue/6));
